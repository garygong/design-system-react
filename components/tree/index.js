...
var Tree = function (_React$Component) {
	...
	function Tree(props) {
		...
		var flattenedNodes = _this.flattenTree({
			nodes: _this.props.getNodes({ nodes: _this.props.nodes }),
			expanded: true
		}).slice(1);
...
var _initialiseProps = function _initialiseProps() {
	...
	this.flattenTree = function (root) {
		...
		if (root.expanded) {
			for (var index = 0; index < root.nodes.length; index++) {
				...
				nodes = nodes.concat(_this2.flattenTree(curNode, treeIndex ? treeIndex + '-' + index : '' + index, false));
			}
		}
		return nodes;
	};

	this.handleSelect = function (_ref) {
	...
	this.handleExpand = function (_ref2) {
