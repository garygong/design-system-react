...
var flattenTree = function flattenTree(root) {
	...
	if (root.expanded) {
		for (var index = 0; index < root.nodes.length; index++) {
			...
			nodes = nodes.concat(flattenTree(curNode, treeIndex ? treeIndex + '-' + index : '' + index));
		}
	}
	return nodes;
};
...
var _initialiseProps = function _initialiseProps() {
	...
	this.handleSelect = function (event, data, clearSelectedNodes) {
	...
	this.handleExpand = function (event, data) {
		_this2.treeHasFocus = true;
